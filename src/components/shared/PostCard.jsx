...
function ReactionIcons({ summary, myReaction }) {
  ...
  return (
    <div
      ...
      style={{ background: 'oklch(0.46 0.15 143)', boxShadow: '0 1px 4px oklch(0.46 0.15 143 / 0.3)' }}
    >
...
function FriendButton({ friendshipStatus, authorUuid, queryKey }) {
  ...
  if (status === 'friends') {
    return (
      <span className="flex items-center gap-1 text-[12px] font-semibold text-primary px-2 py-1 rounded-full" style={{ background: 'oklch(0.46 0.15 143 / 0.08)' }}>
        ...
        <div className="flex mx-1 py-1 gap-0.5">
          ...
          >
            ...
            <button
              ...
              style={isLiked
                ? { color: myReaction?.color ?? 'oklch(0.46 0.15 143)' }
                : { color: '#65676b' }
              }
              onMouseEnter={(e) => {
