    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      ...
      >
        ...
        <div className="p-4 space-y-3 overflow-y-auto" style={{ maxHeight: 'min(65vh, 520px)' }}>
          ...
          {feeling && (
            <div className="inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 fade-in" style={{ background: 'rgba(24,119,242,0.08)', border: '1px solid rgba(24,119,242,0.15)' }}>
            ...
            <div className={`grid gap-1.5 fade-in ${totalImages === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
              {kept.map((url, idx) => (
                <div key={`k-${idx}`} className="relative rounded-xl overflow-hidden aspect-square" style={{ border: '1px solid rgba(0,0,0,0.08)' }}>
                  ...
                  <button onClick={() => setKept((prev) => prev.filter((_, i) => i !== idx))} className="cursor-pointer absolute top-1.5 right-1.5 h-7 w-7 rounded-full bg-black/55 hover:bg-black/75 flex items-center justify-center text-white transition-all duration-200 hover:scale-105">
                    <X className="h-3.5 w-3.5" />
                  </button>
                </div>
              ))}
          ...
          {showFeelings && (
            <div className="rounded-2xl p-3.5 scale-in" style={{ background: dark ? 'rgba(255,255,255,0.05)' : '#fafafa', border: `1px solid ${dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'}` }}>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {FEELINGS.map((f) => (
                  <button
                    ...
                    className={`cursor-pointer flex flex-col items-center gap-1.5 p-2.5 rounded-xl transition-all duration-150 text-center hover:scale-105 ${feeling?.value === f.value ? 'ring-2 ring-[#1877F2]' : dark ? 'hover:bg-white/8' : 'hover:bg-white'}`}
        ...
        <div className="px-4 pb-4 space-y-3">
          <div
            ...
            style={{ border: `1px solid ${dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, background: dark ? 'rgba(255,255,255,0.04)' : '#fafafa' }}
          >
            ...
            <div className="flex items-center gap-0.5">
              ...
              <button onClick={() => fileRef.current.click()} title="Photo" className={`cursor-pointer h-9 w-9 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 ${dark ? 'hover:bg-white/10' : 'hover:bg-gray-200'}`}>
                <ImageIcon className="h-5 w-5 text-green-500" />
              </button>
              <button
                onClick={() => setShowFeelings(!showFeelings)}
                title="Feeling"
                className={`cursor-pointer h-9 w-9 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 ${showFeelings ? dark ? 'bg-yellow-900/30 ring-1 ring-yellow-500/30' : 'bg-yellow-50 ring-1 ring-yellow-200' : dark ? 'hover:bg-white/10' : 'hover:bg-gray-200'}`}
              >
          ...
          <Button
            ...
            style={canPost ? { background: 'linear-gradient(135deg, #1877F2 0%, #4facfe 100%)', boxShadow: '0 4px 14px rgba(24,119,242,0.35)' } : {}}
            disabled={!canPost}
